...
const SEEDED_FLAG = 'prompthub_seeded_v2'
...
【表结构】
CREATE TABLE orders (id INT, user_id INT, amount DECIMAL, created_at DATETIME)
...
【编程语言】Python
【需求描述】实现一个用户注册登录接口
【示例输入（应匹配）】user@example.com
【示例输入（不应匹配）】invalid-email
...
【概念】闭包
【学习者背景】有 Java 基础的前端新手
【希望达到的目标】能看懂 React 源码中的闭包用法
...
【会议主题】Q3 产品路线图评审
【时间/参会人】2024-07-01 14:00，产品+研发+设计 8 人
【原始记录】
（请在此粘贴会议录音转写或笔记）
...
每段标注：情绪、语速、镜头动作、互动话术`,
  },
  {
    ...
    tags: ['客服', '话术', '售后'],
    author: 'PromptHub',
    content: `你是电商客服主管。请生成 智能数码 的客服话术库：
...
export async function seedIfEmpty(): Promise<void> {
  ...
  // 检测旧版 seed flag（v1），需要清理旧种子数据并重新 seed
  const oldFlag = localStorage.getItem('prompthub_seeded_v1')
  if (oldFlag) {
    // 清理旧的种子提示词（author === 'PromptHub'）
    const { queryPrompts, deletePrompt } = await import('./db')
    const all = await queryPrompts({})
    const oldSeeds = all.filter(p => p.author === 'PromptHub')
    for (const p of oldSeeds) {
